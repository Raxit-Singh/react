...
const Accordion = () => {
  ...
  return (
    <div className="flex justify-center items-center text-center w-full h-screen bg-black flex-col">
        {!selected && <button className="bg-slate-500 p-2 rounded-xl text-white " onClick={()=> setMultiSelection(!multiSelection)}>Enable Multi Select</button>}
      <div className="w-[950px]">
        {data.length > 0 ? (
          data.map((dataItem) => (
            <div className="flex justify-center items-center flex-col text-slate-300 mt-3">
              <div
                onClick={multiSelection ? ()=> handleMultiClick(dataItem.id) :() => handleSingleClick(dataItem.id)}
